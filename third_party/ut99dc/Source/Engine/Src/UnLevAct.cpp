...
#if PLATFORM_ANDROID
	debugf( NAME_Log, TEXT("UT99_ANDROID_V238_POSTLOGIN_MUSIC actor=%s actorSong=%s actorSection=%i actorCd=%i transition=%i levelSong=%s levelSection=%i levelCd=%i"),
		Actor->GetFullName(),
		Actor->Song ? Actor->Song->GetFullName() : TEXT("None"),
		Actor->SongSection,
		Actor->CdTrack,
		Actor->Transition,
		GetLevelInfo()->Song ? GetLevelInfo()->Song->GetFullName() : TEXT("None"),
		GetLevelInfo()->SongSection,
		GetLevelInfo()->CdTrack );
#endif
#if PLATFORM_ANDROID
