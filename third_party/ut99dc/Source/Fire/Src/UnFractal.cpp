...
void  UIceTexture::MoveIcePosition(FLOAT DeltaTime)
{
	...
	UDisplace += 2.0f * DeltaTime * -((SBYTE)(HorizPanSpeed - 128));
	VDisplace += 2.0f * DeltaTime *  ((SBYTE)(VertPanSpeed  - 128));
...
void UFireTexture::ConstantTimeTick()
{
	...
	if( (USize>=8) && (VSize>=8) ) // safe sizes ?
	{
		...
		if( GIsPentiumPro )
		{
			if( bRising ) CalcWrapFireP2( &Mips(0).DataArray(0), RenderTable, USize, VSize );
				else      CalcSlowFireP2( &Mips(0).DataArray(0), RenderTable, USize, VSize );
		}
		else
		{
